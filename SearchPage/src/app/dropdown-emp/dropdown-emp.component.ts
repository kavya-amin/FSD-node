...
@Component({
  ...
  templateUrl: './dropdown-emp.component.html',
  styleUrls: ['./dropdown-emp.component.css']
})
export class DropdownEmpComponent implements OnInit {
...
upload() 
{
  ...
  constructor(@Inject(LOCAL_STORAGE) private storage: WebStorageService,private route: ActivatedRoute,private _scrollToService: ScrollToService,private uploadService: FileUploadService,private httpObj: HttpClient,private ser: SearchServiceService, private card: CardsDemoComponent, private router: Router) 
  {     
  ...
  viewAssociate(id: string)
  {
    ...
    console.log(this.uname + " " + this.assEmail);
    this.ser.associateId = id;
    this.associateSearched.length = 0;
    if(this.uname == "admjohn998@gmail.com")
    {
  ...
  else if(this.uname == this.assEmail){
    this.router.navigate(['/star2',id]);
    const config: ScrollToConfigOptions = {
      target: 'destination'
    };
  ...
  this.router.navigate(['/star1',this.assId,'displaySkills',this.assId]));
  const config: ScrollToConfigOptions = {
